...
public class InventoryUtil implements IMinecraft {
    ...
    public static float getHitDamage(ItemStack heldItemMainhand, RemoteClientPlayerEntity fakePlayer) {
        return 0;
    }
    ...
    public static int getElytra() {
        for (int i = 6; i < 45; ++i) {
            if (mc.player.inventory.getStackInSlot(i).getItem() instanceof ElytraItem) {
                return i;
            }
        }
        return -1;
    ...
    public static float getHitDamage(ItemStack stack, net.minecraft.entity.LivingEntity target) {
        if (stack.isEmpty()) return 1.0f;
        float baseDamage = stack.getAttackDamage();
