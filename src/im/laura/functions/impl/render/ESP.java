...
public class ESP extends Function {
    ...
    // --- УНИВЕРСАЛЬНЫЕ МЕТОДЫ ПОЛУЧЕНИЯ HP ---

    private float getUniversalHealth(LivingEntity entity) {
        // 1. Пытаемся взять данные из Скорборда (для FunTime, HolyWorld и т.д.)
        ScoreObjective objective = mc.world.getScoreboard().getObjectiveInDisplaySlot(2);
        if (objective != null) {
            Score score = mc.world.getScoreboard().getOrCreateScore(entity.getScoreboardName(), objective);
            if (score.getScorePoints() > 0) {
                return (float) score.getScorePoints();
            }
        }
        // 2. Если скорборд пуст, берем обычное ХП + золотые сердца
        return entity.getHealth() + entity.getAbsorptionAmount();
    }

    private float getUniversalMaxHealth(LivingEntity entity) {
        float hp = getUniversalHealth(entity);
        float maxBase = entity.getMaxHealth() + entity.getAbsorptionAmount();
        // Если текущее ХП больше базового максимума, считаем текущее ХП за максимум (чтобы не ломать полоску)
        return Math.max(maxBase, hp);
    }
    ...
    public void onDisplay(EventDisplay e) {
        ...
        for (Map.Entry<Entity, Vector4f> entry : positions.entrySet()) {
            ...
            if (entry.getKey() instanceof LivingEntity entity) {
                ...
                if (!remove.getValueByName("Полоску хп").get()) {
                    float hp = getUniversalHealth(entity);
                    float maxHp = getUniversalMaxHealth(entity);
                    float hpOffset = 3f;
                    float out = 0.5f;
                    ...
                    float percentage = MathHelper.clamp(hp / maxHp, 0, 1);
                    DisplayUtils.drawMCVerticalBuilding(position.x - hpOffset, position.y + (position.w - position.y) * (1 - percentage), position.x - hpOffset + 1, position.w, FriendStorage.isFriend(entity.getName().getString()) ? friendColors.w : colors.w, FriendStorage.isFriend(entity.getName().getString()) ? friendColors.x : colors.x);
                }
            }
        }
        ...
        for (Map.Entry<Entity, Vector4f> entry : positions.entrySet()) {
            ...
            if (entity instanceof LivingEntity living) {
                ...
                float hp = getUniversalHealth(living);
                float maxHp = getUniversalMaxHealth(living);
