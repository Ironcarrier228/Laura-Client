...
@FunctionRegister(name = "ElytraTarget", type = Category.Movement)
public class ElytraTarget extends Function {
    ...
    private void useFirework() {
        ...
        if (autofirework.get() && currentTime - lastFireworkTime >= fireworkCooldown) {
            ...
            if (invSlot == -1 && hbSlot == -1) {
                ...
                sendMessage("§c[ElytraTarget] §fФейерверки не найдены!");
                return;
            }
            ...
            if (distanceToTarget > distanse.get()) {
                fireworkCooldown = 300; // Уменьшаем кулдаун, если цель дальше
            } else {
                fireworkCooldown = 200; // Возвращаем изначальный кулдаун
            }
        }
    }
        ...
        if (currentTime - lastChatMessageTime >= chatMessageInterval) {
            if (!targetedPlayers.isEmpty()) {
                ...
                if (target != null) {
                    ...
                    if (mchealth <= 0.01f && deadtoggle.get()) {
                        targetedPlayers.clear();
                        sendMessage("§a[ElytraTarget] §fТаргет был убит! Выключаю модуль");
                        toggle();
                    }
                }
            }
        ...
        if (mc.player != null) {
            ...
            if (save.get() && mchealth < hptoggle.get()) {
                ...
                if (!targetedPlayers.isEmpty()) {
                    ...
                    double deltaY = target.getPosY() - mc.player.getPosY();
                ...
                targetedPlayers.clear();
                stopTargeting();
                sendMessage(String.format("§c[ElytraTarget] §fМодуль выключен, осталось %.1f HP", mchealth));
                toggle();
            }
        }
    }
