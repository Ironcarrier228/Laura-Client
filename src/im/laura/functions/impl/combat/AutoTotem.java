...
@SuppressWarnings({"unused", "Beta", "SameParameterValue"})
public class AutoTotem extends Function {
    private final ModeSetting totemMode = new ModeSetting("Мод", "Обычный", "Обычный", "Funtime");
    private final SliderSetting health = new SliderSetting("Здоровье", 3.5f, 1f, 20f, 0.05f);
    ...
    private final SliderSetting swapDelay = new SliderSetting("Задержка", 50f, 0f, 500f, 10f);
    
    private int nonEnchantedTotems;

    private final ModeListSetting mode = new ModeListSetting("Учитывать", 
    ...
    public AutoTotem() {
        addSettings(totemMode, health, swapBack, saveEnchanted, noBallSwitch, mode, swapDelay);
    }

    @Subscribe
    private void onSpawnEntity(EventSpawnEntity spawnEntity) {
        if (spawnEntity.getEntity() instanceof EnderCrystalEntity && mode.getValueByName("Кристаллы").get()) {
            if (spawnEntity.getEntity().getDistance(mc.player) <= 6.0F) {
                swapToTotem();
            }
        }
    }
    ...
    private void onUpdate(EventUpdate e) {
        ...
        if (totemMode.is("Обычный")) {
            updateNormalMode();
        } else if (totemMode.is("Funtime")) {
            updateFuntimeMode();
        }
    }
    ...
    private void updateNormalMode() {
        ...
        if (shouldToSwapTotem() && slot != -1 && !isTotemInHands()) {
            InventoryUtil.moveItem(slot, 45, handNotNull);
            if (handNotNull && oldItem == -1) oldItem = slot;
    ...
    private void updateFuntimeMode() {
        if (shouldToSwapTotem() && !itemIsHand(Items.TOTEM_OF_UNDYING)) {
            swapToTotem();
        }
        swapBack();
    }
    ...
    private void swapBack() {
        if (stopWatch.isReached(swapDelay.get().longValue()) && itemIsBack()) {
            itemInMouse = -1;
            backItem = Items.AIR;
            backItemStack = null;
            stopWatch.reset();
    ...
    private void swapToTotem() {
        ...
        if (mainHandItem == Items.TOTEM_OF_UNDYING) return;
        if (totemSlot == -1 && !isCurrentItem(Items.TOTEM_OF_UNDYING)) return;

        if (itemInMouse == -1) {
            itemInMouse = totemSlot;
            backItem = mainHandItem;
    ...
    public int countTotems(boolean includeEnchanted) {
        ...
        for (int i = 0; i < mc.player.inventory.getSizeInventory(); ++i) {
            ...
            if (slotStack.getItem() == Items.TOTEM_OF_UNDYING && 
                (includeEnchanted || !slotStack.isEnchanted())) {
                ++totemCount;
            }
        }
    ...
    private boolean shouldToSwapTotem() {
        ...
        if (!isOffhandItemBall() && isInDangerousSituation()) {
            return true;
        }
    ...
    private boolean isInDangerousSituation() {
        return checkCrystal() || checkTnt() || checkAnchor() || checkFall();
    }

    private boolean checkFall() {
        if (!mode.getValueByName("Падение").get()) return false;
        if (mc.player.isInWater() || mc.player.isElytraFlying()) return false;
        return mc.player.fallDistance > 10.0f;
    }

    private boolean checkTnt() {
        if (!mode.getValueByName("ТНТ").get()) return false;
    ...
    private boolean isOffhandItemBall() {
        boolean isFallingConditionMet = mode.getValueByName("Падение").get() && mc.player.fallDistance > 5.0f;
        if (isFallingConditionMet) return false;
    ...
    private boolean isDangerousEntityNearPlayer(Entity entity) {
        return (entity instanceof TNTEntity || entity instanceof TNTMinecartEntity) && 
               mc.player.getDistance(entity) <= 6.0F;
    }
