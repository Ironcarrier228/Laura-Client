...
public class ViaMCP {
    public final int NATIVE_VERSION = 754; // 1.16.5
    public final List<ProtocolVersion> PROTOCOLS;
    public final VersionSelectScreen viaScreen;

    public ViaMCP() {
        List<ProtocolVersion> protocolList = ProtocolVersion.getProtocols().stream()
            .filter(pv -> pv.getVersion() == 47 || (pv.getVersion() >= 107 && pv.getVersion() <= 774))
        ...
        ViaLoadingBase.ViaLoadingBaseBuilder.create().runDirectory(new File(Laura.getInstance().getFilesDir().getName())).nativeVersion(754).build();
