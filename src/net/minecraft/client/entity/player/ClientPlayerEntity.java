...
import im.laura.command.CommandDispatcher;
import im.laura.command.impl.DispatchResult;
import im.laura.events.*;
import im.laura.functions.api.FunctionRegistry;
import im.laura.functions.impl.misc.AntiPush;
import im.laura.functions.impl.misc.SelfDestruct;
import im.laura.functions.impl.movement.AutoSprint;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            if (!Laura.getInstance().playerOnServer) {
                Laura.getInstance().playerOnServer = true;
            }

            Laura.getInstance().getEventBus().post(eventUpdate);
            super.tick();
    ...
    private void onUpdateWalkingPlayer() {
        ...
        Laura.getInstance().getEventBus().post(ea);
        ...
        if (this.isCurrentViewEntity()) {
            ...
            Laura.getInstance().getEventBus().post(eventMotion);
        ...
        Laura.getInstance().getEventBus().post(new EventPostMotion());
    ...
    public void sendChatMessage(String message) {
        ...
        CommandDispatcher commandDispatcher = Laura.getInstance().getCommandDispatcher();
    ...
    public void closeScreen() {
        ...
        Laura.getInstance().getEventBus().post(event);
        if (!event.isCancel()) {
    ...
    public void setSprinting(boolean sprinting) {
        Laura.getInstance().getEventBus().post(sprintEvent);
    ...
    public void livingTick() {
        ...
        if (this.isHandActive() && !this.isPassenger()) {
            Laura.getInstance().getEventBus().post(noSlowEvent);
